...
// Retries result-returning function for up to 10 times
// until an expected status is returned, waiting for up to 1s
// between retries
export function retryUntilExpected(expected, f) {
  for (let i = 0; i < 9; i++) {
    const res = f()
...
export function pollTimeoutUntilExpected(expected, timeoutMs, f) {
  const timeWas = new Date();
  let timeSpent = null
  let res = null
  // Poll schemaDefinition until receiving a 200
  while (new Date() - timeWas < timeoutMs) {
    res = retryUntilExpected(expected, () => { return f() })
    if (res) {
