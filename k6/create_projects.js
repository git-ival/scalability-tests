...
export function getPrincipalIds(cookies) {
...
export function getMyId(cookies) {
  const response = http.get(
...
export function getClusterIds(cookies) {
  const response = http.get(
...
export function createProjects(data, iter) {
  let response
  const i = iter ? iter : exec.scenario.iterationInTest
